...
namespace Passingwind.UserDialogs.Platforms
{
    ...
    public class PromptFormAppCompatDialogFragment : AbstractAppCompatDialogFragment<PromptFormConfig>
    {
        protected override void OnKeyPress(object sender, DialogKeyEventArgs args)
        {
            base.OnKeyPress(sender, args);
        ...
        protected override Dialog CreateDialog(PromptFormConfig config)
        {
            return new PromptBuilder().BuildForm(this.AppCompatActivity, config);
        }
        ...
        protected virtual void SetAction(bool ok)
        {
            try
            {
                ...
                // this.Config?.OnAction(new PromptFormConfig(ok, txt.Text.Trim()));
                this.Dismiss();
            }
            catch { } // swallow
        }
